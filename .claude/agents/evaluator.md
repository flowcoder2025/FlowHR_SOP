...
| 기준 | 가중치 | 설명 |
|------|--------|------|
| 완성도 (Completeness) | 30% | 필수 섹션 빠짐 없음, 산출물 디렉토리 매핑 일치, 빈 자리/누락 없음. **전역 컴포넌트(헤더 드롭다운·사이드바·모달·푸터·헬프)·라우팅 매트릭스(진입점·역할별·권한 미일치·세션 만료)·전이 동선(화면 A→B)·시스템/정적 페이지(약관·온보딩·PWA 설치) 명세 모두 존재 의무** (KI-031, 2026-05-15 보강) |
...
- **(2026-05-15 추가, KI-031)** 화면 명세는 있으나 글로벌 헤더 드롭다운(프로필·알림·검색·도움말) 명세 부재
- **(2026-05-15 추가, KI-031)** 진입점(`/`) / 역할별 라우팅 매트릭스 부재
- **(2026-05-15 추가, KI-031)** 화면 간 전이 동선 (A→B) 명시 부재 (PRD §9 의존성에 명시되어야 함)
- **(2026-05-15 추가, KI-031)** 운영사 본인 프로필·약관·온보딩·PWA 설치 등 정적 페이지 누락
