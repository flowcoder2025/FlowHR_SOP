...
| KI-110 | P3 | 7 | DB | `claim_due_scheduled_setting_changes`(security definer) 의 UPDATE 가 service_role 컨텍스트에서 audit 트리거 발화 → `audit_logs.actor`(auth.uid())가 NULL 기록(cron/시스템 행위자 식별 불가). 데이터 손실/보안 영향 없음(시스템 행위로 복원 가능). KI-106(audit 페이로드) 인접 — cron apply 로직 구현(WI-032) 시 시스템 actor 표기 보강 | evaluator WI-031 듀얼검증 | 2026-06-01 | scheduled (WI-032 cron apply 또는 audit 보강 batch) |
