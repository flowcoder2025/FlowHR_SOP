...
| P2 Medium | **6** | 5 | ✅ 트리거 도달 — 활성: KI-054 / KI-061 (둘 다 **Phase 7 React 변환 scheduled** — IconButton aria-label / Stepper 컴포넌트화, KI-061은 WI-030으로 OP-04 부분 해소) + **KI-079** (WI-020 rememberMe 세션 TTL 미반영, 세션관리 ST-005) + **KI-092** (WI-020-2 ST-078 AC-5 운영사 감사 미구현, OP-09 audit WI) + **KI-094** (WI-020-3 maintenance message_en 컬럼 부재 — 운영사 점검 본문 ko 단일, 글로벌 출시 전) + **KI-109** (WI-031 검출 — accept_invitation security-definer 함수 anon/authenticated EXECUTE 잔존, token-gated, 보안 하드닝 WI sweep). **WI-KI-batch-008-wf resolved: ~~KI-049~~ (권한매트릭스 15화면) + ~~KI-060~~ (TA-13 font-weight)**. (이전 resolved: ~~KI-050/051/053/058/063/071/076~~) |
| P3 Low | **51** | 10 | ✅ 트리거 도달 — 활성: KI-005/006/007/016/017/020/023/025/035/036/045/055/066/067/072/073 + **WI-020: KI-080/081/082/083** + **WI-019 Day8~10: KI-084/085/086/087** + **WI-021: KI-088/089** + **WI-021-1: KI-090** + **WI-020-2: KI-091** + **WI-020-3: KI-093/095/096** + **WI-020-4: KI-097/098** + **WI-020-5: KI-099/100/101/102** + **WI-020-6: KI-103/104/105/106** + **WI-030: KI-107/108** + **WI-031: KI-111** + **WI-032: KI-112** (leave_policy grant_basis 저장 위치) + **Phase 7 scheduled 재분류 6건 (batch-008): KI-038/054(P2)/057/061(P2)/064/065/068/070 svg·aria·모달·stepper·반응형 — React 변환 시 해소**. **WI-032 resolved: ~~KI-110~~ (audit 시스템 actor fallback)**. **WI-KI-batch-008-wf resolved: ~~KI-043~~(CM-21 i18n) + ~~KI-044~~(CM-04 selector) + ~~KI-069~~(권한매트릭스 표 구조 15화면)**. (batch-007 resolved 8건 + 이전 분) |
...
| KI-061 | P2 | 5 | Wireframe | components.css stepper/step 중복 + **batch-008 추가 발견: EM-03/EM-08 stepper가 미정의 클래스 `.step-body`/`.step-title`/`.step-sub` 사용 (components.css 정의 0건) → 미스타일 렌더**. `.tab.is-active`/`.modal-header`/`.vert-tab` 중복은 이전 hotfix 해소 확인. 잔존: `.step`/`.stepper` G2 dead code(L509-522) + 미정의 step 자식 클래스. | evaluator G3 hotfix3 + codex + batch-008 검증 | 2026-05-18 | **부분 해소 (WI-030 2026-06-01 — packages/ui `Stepper` 컴포넌트가 G3 list 형식을 SSOT로 채택, OP-04 7단계 마법사 좌측 네비 커버. G2 카드형 dead code는 React 변환으로 폐기). 잔존: EM-03/EM-08 2단 변종(`.step-body`/`.step-title`/`.step-sub` = title+sub)은 Stepper에 미반영 — EM-03=Sprint 5 / EM-08=Sprint 7 소유. 해당 WI에서 `summary`/`sub` 필드 추가 또는 별도 `StepperCard` 변형으로 처리** |
...
| KI-097 | P3 | 7 | Code | ST-002 비밀번호 재설정의 **실메일 발송 + cross-device 링크 클릭(/auth/confirm verifyOtp → recovery 세션 → updateUser → signOut global)** 전체 흐름이 자동 E2E 미검증 — Free hosted Supabase 기본 SMTP 발송 제약 + 대시보드 템플릿 의존. forgot 동일응답(sent)/reset 세션없음 만료안내/스키마·비번정책 unit+E2E 로 핵심 커버(KI-089/091/093 동류). 커스텀 SMTP 또는 메일 inbox 캡처 E2E 인프라 시 보강 | claude+codex WI-020-4 | 2026-05-29 | scheduled (KI-089 e2e 시드/메일 인프라 동반) |
| KI-098 | P3 | 7 | Code | 비밀번호 재설정 token_hash 흐름은 **원격(staging/prod) Supabase 대시보드 Recovery 이메일 템플릿**이 `/auth/confirm?token_hash={{ .TokenHash }}&type=recovery&redirect_to={{ .RedirectTo }}` 를 가리키도록 + Redirect URL allow-list 에 배포 도메인/`/{locale}/reset-password` 등록 **수동 설정** 전제(MCP/CLI 범위 외). 로컬은 config.toml+templates/recovery.html 로 커밋. 미설정 시 기본 ConfirmationURL(PKCE) → cross-device 실패. 사용자 대시보드 조치 필요(KI-086 류) | claude+codex WI-020-4 | 2026-05-29 | scheduled (베타/배포 도메인 확정 시 대시보드 설정) |
| KI-099 | P3 | 7 | Code | 2FA 전용 env 키 `AUTH_TOTP_ENC_KEY`/`AUTH_CHALLENGE_SECRET`(각 32바이트 base64)가 **원격(staging/prod Vercel) env 에 수동 프로비저닝 필요**(MCP/CLI 범위 외, KI-086/098 류). 로컬 `.env.local` + `.env.example` 문서화 완료. 미설정 시 2FA 흐름 fail-closed(키 부재 throw) — 2FA 사용자 로그인/관리 차단. service_role 겸용 금지(유출/회전 파급) | claude WI-020-5 | 2026-05-29 | scheduled (배포 도메인 확정 시 Vercel env 설정) |
| KI-100 | P3 | 7 | Code | 2FA 전체 흐름 E2E(enable→재로그인 challenge→TOTP/복구코드→disable)가 `E2E_TEST_EMAIL`+service-role env 게이트 + `workers:1` 직렬 + test-employee totp 컬럼 service-role teardown 의존(공유 시드 계정 변경 → 병렬 시 login.spec 충돌, KI-082/089 동류). 화면 가드 3 E2E 는 무조건 실행. CI 자동 회귀는 secrets 주입 시 활성 | claude WI-020-5 | 2026-05-29 | scheduled (KI-082 secrets 활성 + e2e 시드 인프라 동반) |
| KI-101 | P3 | 7 | Code | 2FA challenge 쿠키(`fh-2fa-challenge`)의 `jti` 서버측 단일사용 소비 미구현 — 만료(5분) 전 쿠키 복사 재생 이론상 가능. 완화: HttpOnly+Secure+5분 + 재생에 쿠키 탈취+유효 OTP 동시 필요. 동일 성질 WI-020-4 recovery-marker(HMAC 15분)도 수용된 선례. 완전 차단은 **서버측 jti 저장소(DB 컬럼/테이블 = schema 변경, §5/§7-2 사용자 승인)** 필요 → 후속 인증 하드닝 WI(KI-078 인접) | codex WI-020-5 듀얼검증 | 2026-05-29 | scheduled (인증 하드닝 WI — 사용자 승인 동반 DB jti 저장소) |
| KI-102 | P3 | 7 | Code | `/me/security` 2FA 관리(disable 비번+코드 재확인 / enroll 6자리 확인)가 login_attempts 잠금/레이트리밋 미연동. 위협 완화: **이미 인증된 세션 + 필수 2차 인증(TOTP/복구코드)** 이중 게이트(pre-auth 브루트포스 KI-078 대비 낮음) + enroll 은 10분 setup 쿠키 바운드. CM-04/§-0h 스펙 위반 아님(잠금은 /two-factor 한정 명시). evaluator 는 disable 을 P2/P3 로 제시 — 이중 게이트 근거로 P3 분류 | evaluator WI-020-5 듀얼검증 | 2026-05-29 | scheduled (인증 하드닝 WI — per-session 재확인 카운터) |
| KI-103 | P3 | 7 | Code | 초대 활성화 이메일(welcome_invite) 실발송 미구현 — createInvitation 이 activation URL 을 반환하는 추상화 단계까지만(Resend/SMTP 미설정, guardrails §9 외부비용 무단산입 금지). OP-04/TA-02 인비터 UI + 실발송은 후속 WI. 현재 staging/E2E 는 invitations 직접 생성 + URL 직접 구성으로 검증 | claude WI-020-6 | 2026-06-01 | scheduled (Resend 설정 + OP-04/TA-02 인비터 UI WI) |
| KI-104 | P3 | 7 | Code | 활성화 보상 삭제(admin.createUser 성공 후 accept_invitation rpc 실패 시 deleteUser)가 그 deleteUser 마저 실패하면 orphan auth.users 잔존(삼중 실패, 매우 드묾). accept_invitation 은 SQL 트랜잭션이라 public 측은 자동 롤백(부분상태 없음) — auth.users 만 잔여 가능. 운영 orphan 스캐너/정리 cron 으로 보강 | codex WI-020-6 듀얼검증 | 2026-06-01 | scheduled (운영 관측성/정리 cron Phase 10 인접) |
| KI-105 | P3 | 7 | Code | CM-03 명세(common.md §CM-03 "선택적 2FA 설정" + wireframes/analysis/CM-03.md state two_fa)의 **활성화 in-flow 직원 선택 2FA 셋업** 단계 미구현 — 운영사는 활성화 후 /me/security?forced=2fa 강제 리다이렉트로 처리, 직원은 활성화 후 /me/security 에서 선택 enable(ST-004 재사용) 가능하나 활성화 화면 내 단계는 없음. API auth.md activate 의 totpSetup 은 optional 이라 차단 아님 | evaluator WI-020-6 듀얼검증 | 2026-06-01 | scheduled (활성화 UX 보강 — CM-03 in-flow 2FA WI) |
| KI-106 | P3 | 7 | Code | `activate/actions.ts` 의 `user.activated` audit 페이로드가 2곳(세션수립 실패 경로 + 정상 경로)에 동일 복사 — evaluator P3(activatedAudit 단일 추출 권고). 정정 커밋(1bf3fef)이 작성됐으나 PR #44 squash 머지에 미반영(754898d 까지만 머지). **기능 무해**(동일 페이로드, 한쪽만 실행) + 코드 중복 2회(임계 3회 미만)라 차단 아님. 핵심 P2(inet clientIp)는 머지 반영됨. 사용자 결정(2026-06-01): KI 등록 후 다음 인증 WI/cleanup batch 에서 정리 | claude WI-020-6 사후점검 | 2026-06-01 | scheduled (다음 인증 WI 또는 cleanup batch) |
| KI-107 | P3 | 7 | Code | packages/ui `Stepper`가 03-components.md:682-688 의 완료 단계 `summary` 필드(예 "(주)치킨매니아" 요약 표시)를 미반영 — 현재 OP-04.html 은 step-label 만 사용해 즉시 차단 아님. OP-04 완료/검토 단계 요약 표시 요구 발생 시 `StepperStep`에 `summary?` 필드 추가 | evaluator WI-030 듀얼검증 | 2026-06-01 | scheduled (WI-036 OP-04 마법사 UI에서 필요 시 props 확장) |
| KI-108 | P3 | 7 | Code | packages/ui primitive 의 클릭/키보드 콜백 발화(onStepClick/onSortChange/onRowClick/VerticalTabs.onChange/FilterPanel.onClear)가 자동 검증 미적용 — 검증은 vitest node + react-dom/server renderToStaticMarkup(정적 markup + aria + nextSortState 순수함수)으로 커버하나 실제 이벤트 핸들러 발화는 jsdom interaction test 필요(KI-082/089 e2e 인프라 동류). 도메인 컴포넌트 양산(Sprint 3~6) 시 @testing-library/react+jsdom 도입으로 보강 | codex+evaluator WI-030 듀얼검증 | 2026-06-01 | scheduled (jsdom interaction test 인프라 도입 WI) |
| KI-109 | P2 | 7 | DB | `accept_invitation`(mig 35, security definer)의 EXECUTE 가 Supabase pg_default_acl 로 **anon/authenticated 에 잔존**(staging proacl 실측 anon_exec=true/auth_exec=true) — WI-031 claim 함수와 동일 클래스(`revoke all from public`만으론 무력). service_role 의도이나 임의 인증/익명 호출 가능. **token_hash 게이트(고엔트로피 1회용)로 실 노출 제한** — 토큰 없이는 무해. defense-in-depth + 동일 클래스 sweep 필요. ※ `get_invitation_by_token_hash` anon=true 는 비인증 /activate 검증용 **의도된 설계**(제외). 조치: `revoke execute on function accept_invitation(...) from public, anon, authenticated`(mig 31/39 패턴) | evaluator WI-031 듀얼검증 | 2026-06-01 | scheduled (인증/보안 하드닝 WI — service-only security-definer 함수 grant 일괄 감사) |
| ~~KI-110~~ | P3 | 7 | DB | ~~`claim_due_scheduled_setting_changes`(security definer) 의 UPDATE 가 service_role 컨텍스트에서 audit 트리거 발화 → `audit_logs.actor`(auth.uid())가 NULL 기록~~ | evaluator WI-031 듀얼검증 | 2026-06-01 | **resolved (WI-032 mig 40 — `audit_row_change` 에 `app.audit_actor_*` GUC fallback[auth.uid() NULL 일 때만, 기존 경로 무영향]. apply 엔진이 row별 `set_config`(예약자 created_by + `system:scheduled-settings`). staging 실증: company 적용 audit `actor_id=예약자/actor_role=system:scheduled-settings`)** |
| KI-112 | P3 | 7 | DB | `leave_policy.grant_basis`(부여 기준: 입사일/회계연도) 저장 위치 부재 — `tenant_settings` 에 `company_info/notification_config/security_policy` jsonb 3개뿐(mig 11), 휴가정책 전용 컬럼 없음. WI-032 는 `leave_types` upsert/delete 만 구현(grant_basis 제외). `company_info` 에 혼입은 의미 부정합, 신규 jsonb 컬럼 추가는 schema 변경(§5/§7-2 사용자 승인). 조치: `tenant_settings.leave_policy jsonb` 컬럼 추가 또는 전용 저장 후 leave_policy payload 에 grant_basis 포함 | codex+claude WI-032 협의 | 2026-06-01 | scheduled (휴가정책 저장 schema 보강 WI — 사용자 승인 동반) |
| KI-111 | P3 | 7 | Code | `workPolicySchema.applicable_departments` = `z.array(uuidSchema)` 이나 DB(mig 11)·database.ts 는 `text[]`(string[]) — 비-UUID 부서키 Row 가 런타임 zod 검증 실패. **WI-021-1(commit 9c49713) 도입분, WI-031 diff 밖**. 스키마-DB Row 정합 위반(KI-090 류). 조치: z.array(z.string()) 정정 | evaluator WI-031 듀얼검증 | 2026-06-01 | scheduled (후속 schema 보강 WI — KI-090 동반) |
