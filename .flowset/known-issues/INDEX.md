...
|--------|----------|------------|-----------|
| P0 Critical | 0 | 1 | ❌ |
| P1 High | **2** | 3 | ❌ 임계 미달 — 활성: **KI-077** (WI-019 교차테넌트 FK, ST-005 Day8) + **KI-078** (WI-020 분산 무차별대입 하드닝 — CAPTCHA+per-IP 429+TOCTOU, 인증 하드닝 WI, 사용자 defer 2026-05-29). (KI-046/047/048 batch-006-fix3-rev1 resolved + audit hotfix3 정정 2026-05-18) |
| P2 Medium | **3** | 5 | ❌ 임계 미달 — 활성: KI-054 / KI-061 (둘 다 **Phase 7 React 변환 scheduled** — IconButton aria-label / Stepper 컴포넌트화) + **KI-079** (WI-020 rememberMe 세션 TTL 미반영, 세션관리 ST-005). **WI-KI-batch-008-wf resolved: ~~KI-049~~ (권한매트릭스 15화면) + ~~KI-060~~ (TA-13 font-weight)**. (이전 resolved: ~~KI-050/051/053/058/063/071/076~~) |
| P3 Low | **26** | 10 | ✅ 트리거 도달 — 활성: KI-005/006/007/016/017/020/023/025/035/036/045/055/066/067/072/073 + **WI-020: KI-080/081/082/083** + **Phase 7 scheduled 재분류 6건 (batch-008): KI-038/054(P2)/057/061(P2)/064/065/068/070 svg·aria·모달·stepper·반응형 — React 변환 시 해소**. **WI-KI-batch-008-wf resolved: ~~KI-043~~(CM-21 i18n) + ~~KI-044~~(CM-04 selector) + ~~KI-069~~(권한매트릭스 표 구조 15화면)**. (batch-007 resolved 8건 + 이전 분) |
...
| KI-077 | P1 | 7 | DB | tenant-scoped 자식 테이블(leaves 등)의 tenant_id/employee_id/leave_type_id가 독립 FK라 DB 레벨에서 "동일 테넌트 부모" 미강제 → 교차 테넌트 UUID 조합 가능 (멀티테넌트 격리 defense-in-depth 갭). RLS(WITH CHECK)가 일차 격리이나 employee_id 교차참조엔 추가 강제 필요. composite FK(`(tenant_id,id)` 참조) vs RLS-only를 ST-005에서 일괄 결정 | codex WI-019 듀얼검증 | 2026-05-28 | scheduled (ST-005 Day 8 RLS — 사용자 defer 결정 2026-05-28. staging 비어있는 비프로덕션) |
