...
## 2. 화면별 가용성 매트릭스 (44 화면 × 3 클라이언트)

> 36 → 44 (CM-16~22, OP-12 추가, 2026-05-15 KI-027~030 batch-003).
...
| OP-11 | 시스템 설정 | ✓ | — | ✓ | — |
| OP-12 | 운영사 본인 프로필 | ✓ | △ | ✓ | PWA는 프로필 + 알림 토글, 세션 관리는 비권장 |
...
### 공통 (CM-01 ~ CM-22) — 신규 헤더·정적 페이지 매트릭스

| ID | 화면 | Web | PWA | Tauri | 비고 |
|----|------|:---:|:---:|:---:|------|
| CM-01~07 | 인증·알림 기본 화면 | ✓ | ✓ | ✓ | (기존, 변경 없음) |
| CM-08 | 공통 검색 (v1.1) | △ | △ | △ | v1.1 활성 |
| CM-09~15 | 파일·엑셀·PDF·감사·알림 발송 | ✓ | ✓ | ✓ | (기존) |
| CM-16 | 헤더 프로필 드롭다운 | ✓ | ✓ | ✓ | 모바일은 햄버거 → 사이드바 내 동일 메뉴 |
| CM-17 | 헤더 알림 종 드롭다운 | ✓ | ✓ | ✓ | Tauri는 트레이 배지와 연동 |
| CM-18 | 헤더 검색 | △ | — | △ | PWA는 헤더 검색바 숨김 (햄버거 메뉴 안 검색 항목), MVP는 안내 토스트 |
| CM-19 | 헤더 도움말 | ✓ | △ | ✓ | PWA는 도움말 슬라이드 패널 풀스크린 |
| CM-20 | PWA 설치 가이드 | △ | ✓ | — | Tauri는 자체 앱이라 노출 안 함 |
| CM-21 | 약관/개인정보처리방침 | ✓ | ✓ | ✓ | 가독성 위해 PWA는 폰트 확대 |
| CM-22 | 첫 사용자 온보딩 투어 | ✓ | ✓ | ✓ | 디바이스별 모달 크기 자동 조정 |
...
| 2026-05-15 | 44화면으로 확장 (CM-16~22, OP-12) + 신규 매트릭스 | KI-027~030 batch-003 |
