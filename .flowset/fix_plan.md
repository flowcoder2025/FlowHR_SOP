...
- [x] WI-030-feat packages/ui 도메인 primitive 5종 (Stepper/DataTable+RowLink+rowHighlight+nextSortState/FilterBar+FilterChip+FilterPanel/DomainPrefixInput/SettingsPane+VerticalTabs) — Phase 5 `_design-system/components.css` SSOT React 변환, 비즈니스 로직 없는 reusable primitive. KI-061 **부분 해소**(Stepper G3 list SSOT 채택 → OP-04 7단계 마법사 커버, G2 dead code 폐기. EM-03/EM-08 2단 변종은 Sprint 5/7 잔존). 검증 인프라(vitest node + react-dom/server renderToStaticMarkup, jsdom 불요) 신규 + 단위 27. **듀얼검증 PASS_BOTH**(evaluator 8.60 / codex WARNING P2×3 a11y → hotfix 7a48256[정렬·행 키보드 접근 + VerticalTabs nav 패턴 + fallback 가드 + Stepper currentIndex] → PASS_VERIFIED). turbo lint/typecheck/test/build 21/21. KI-107(Stepper summary 필드)/108(콜백 jsdom interaction 미검증) 등재. PR #47.
- [x] WI-031-feat Sprint 2 DB foundation (마이그레이션 36~39, 사용자 승인 + 적용 전 SQL 재확인 후 staging 적용) — **36** tenant_drafts status enum+운영자당 열린 draft 1개 partial unique(R-마법사)+step 1~7 체크 / tenants 회사명 trgm GIN+status·plan·updated_at 인덱스+slug 소문자 unique / work_policies 테넌트당 기본 1개 unique / document_templates (tenant,key) unique / approval_lines 인덱스. **37** scheduled_setting_changes 테이블(적용일 예약 큐)+enum+RLS 3정책(operator/tenant_admin)+claim_due_scheduled_setting_changes() service_role+audit 트리거. **38** plans seed 3종(기본 ₩9,900/명·프리미엄 ₩19,800/명·커스텀, **OP-04 와이어프레임 SSOT** — placeholder 아님, 사용자 확인). codex 권고 중 roles seed(권한 모델 Sprint 9 EP-06, app 미소비)·Sprint 8 인덱스는 추측 배제로 제외. 입점사 맞춤 모듈은 기존 feature_flag_overrides(mig 8)로 지원 — 추가 스키마 불요. database.ts 재생성 + zod 엔티티 2종(tenantDraft 갱신/scheduledSettingChange 신규)+enum 2종 + OpenAPI 등록. **듀얼검증 PASS_BOTH**(evaluator 1차 FAIL 7.75[P1: claim 함수 security-definer 의 anon/authenticated EXECUTE 잔존 → cross-tenant payload 유출+cron DoS, codex 놓침] → hotfix mig 39[revoke from public/anon/authenticated, mig 31 패턴 + update 정책 축소 pending→pending/cancelled] → 재평가 PASS 8.80 / codex WARNING P2[update 정책]→PASS). proacl 실측 {postgres,service_role} + rls_matrix T14/T15 staging PASS + turbo 21/21 + schemas 53. KI-109(P2 accept_invitation 동일 클래스 grant, 보안 하드닝 sweep)/110(P3 claim audit actor NULL)/111(P3 applicable_departments zod text[], WI-021-1) 등재. PR #48.
- [x] WI-032-feat TA-13 회사설정 API (GET 9탭 envelope + PATCH P0 4탭 + 적용일 즉시/예약 pg_cron 엔진) [ST-053] — codex 2라운드 협의(인터페이스=Server Action+lib, REST route handler 미신설 / cron=pg_cron DB-side / 재시도·복구 정책). **mig 40** 예약 적용 엔진: `audit_row_change` 시스템 actor fallback(app.audit_actor_* GUC, auth.uid() NULL 시만 → **KI-110 resolved**) / `claim_due` backoff(1m/5m/15m/1h)+attempt cap(5) / `apply_one`(즉시 원자 claim+apply, service_role) / `_apply_claimed`(target별 payload→테이블) / `recover_stale`(15분) / `run_due`(cron entrypoint) / pg_cron 설치+cron.schedule(매분 적용+audit 보관 재등록). 신규 5함수 anon/auth EXECUTE revoke(KI-109 클래스 재발 없음, proacl {postgres,service_role} 실측). **mig 41** hotfix: scheduled_setting_changes INSERT RLS 를 P0 4 target 제한(codex P2). schemas `tenant-settings.ts`(P0 4탭 payload zod snake_case strict + 단위 27, OpenAPI 54 defs) + lib `tenant-settings/{queries 9탭 envelope, actions 즉시/예약, permissions 순수권한+단위}` + database.ts 재생성. **듀얼검증 PASS_BOTH**(evaluator 8.45 / codex 1차 WARNING[P1 hr_admin 권한과다+P2 큐 INSERT target] → hotfix 31c9271[canRead security/roles super 전용 + mig 41] → 재검증 PASS). staging 전 시나리오 실증(즉시/예약/run_due/실패재시도/stale복구/audit actor) + turbo 21/21. KI-112~116 등재. PR #50.
- [x] WI-033-feat TA-13 회사설정 UI (SettingsPane 9탭 shell + 회사정보/근무/휴가/결재라인 4탭 폼 + E2E) [ST-053] — codex 2라운드 협의(mig 40 apply 엔진 정합 검증) 단일안. 라우트 `(tenant)/admin/settings`: 세션+tenant 가드 → WI-032 `getTenantSettings` 9탭 envelope → `SettingsClient`(SettingsPane+VerticalTabs WI-030). P0 4탭 폼 useActionState(react-hook-form 미도입, 기존 8폼 패턴) — company full-replace 전체제출 / work 시간 optional+late_threshold 게이트 / leave 인라인 테이블+delete_keys / approval 기본필드+conditions 병합보존(WI-034 소유). 즉시/예약 공통 ActionBar(apply_at KST +09:00 정규화, 과거거부)+탭별 pending 이력. read-only 5탭(ReadonlyPane+AuditLogsPane). URL ?tab= 동기화(history.replaceState, 탭 key remount). `lib/tenant-settings/form-data.ts` 순수변환(단위 17)+`tabs.ts`+i18n `screens.ta-13` ko/en(106 leaf). KI-113 사용자 결정 현행 유지(hr_admin 보안/역할권한 super 전용). **듀얼검증 PASS_BOTH**(evaluator 1차 8.75→재검증 8.63 / codex 1차 FAIL[P1 approval 조건 클라위조+scheduled UPDATE RLS / P2 leave FK 실패큐+JSON 대량삭제] → hotfix c4f6dcb[원본 서버 DB 권위조회+strict JSON+FK 사전검사] → CONDITIONAL[DB조회 fail-open] → hotfix 8943265[세 조회 fail-closed] → PASS). ★ codex 가 evaluator 통과분에서 P1×2 검출 → 듀얼검증 가치 재실증. turbo 21/21+form-data 17+E2E 가드 2 PASS. KI-117(P2 scheduled UPDATE RLS cancel 전용 축소, 보안 sweep)/118(P3 apply_at 이중경계)/119(P3 mutation E2E admin 시드) 등재. PR #52.
- [x] WI-034-feat 결재라인 조건 분기 (approval_lines.conditions jsonb 평가엔진 + 조건트리 UI + E2E) [ST-054] — codex 2라운드 협의 확정(DB/RLS 변경 없음). `packages/schemas/src/approval-line-dsl.ts`(신규) DSL zod(snake_case) + 순수 평가엔진 `resolveApprovalLine` SSOT: step{order,approver_role(테넌트 4역할),dept_scope,specific_employee_id}, condition{field(enum 5),op,value,line.min(1)}, field-op 매트릭스(숫자op=leave_days 전용/문자 field=`==,!=,in,not_in`), order 위치정합 1..n, 활성 라인 default_line≥1, 숫자 coerce 금지, 미존재/무효 actual fail-closed. 스키마 승격(payload+entity z.unknown()→strict DSL). form-data 조건 직접편집(order=위치)+collectSpecificEmployeeIds, actions strict 검증+unknown line id fail-closed+specific_employee_id 테넌트 소속검증, queries 결재자 picker(employees/departments) RLS 조회, 조건트리 편집 UI(라인 삭제=is_active=false 비활성화 반영). i18n ko/en 확장+에러 3키. 정의 audit 은 mig 29 트리거 자동. **듀얼검증 PASS_BOTH**(evaluator 8.72 / codex 1차 P2×2+P3×2 → hotfix 94742e1[isValidActual 가드·order 위치정합·strictNumber·entity refine·라인삭제 비활성화] → 재검증 PASS). turbo 21/21 + 단위(approval-line-dsl 27/tenant-settings 30/form-data 22) + openapi 54 defs. KI-120(mig40 affected-row-0)/121(ST-046 소비 재검증)/122(matrix.json status drift) 등재. 실 요청시점 분기 적용=ST-046(Sprint 6). PR #53.
- [x] WI-035-feat OP-04 등록 API (check-domain/business-number 검증 + drafts upsert + 최종 등록 트랜잭션 + 관리자 초대) [ST-006] — codex 2라운드 협의 + 사용자 승인(DB mig 42). **마이그레이션 42** `register_tenant` SECURITY DEFINER 원자 RPC(service_role only, KI-109 grant 패턴 staging 실측 {postgres,service_role}): tenants+subscriptions+tenant_settings+초기데이터(departments/work_policies/leave_types/approval_lines/document_templates)+관리자 invitation 단일 트랜잭션. **create-at-activate 정합**(DoD "admin user INSERT" → invitation 행으로 재해석, WI-020-6 최신 계약): 등록 시 invitation 만, 관리자 user 는 활성화 시점 생성. `accept_invitation` 최소확장(tenant_super 수락 시 admin_user_id backfill + 활성화 audit actor). **enum 무변경**(표시상태 admin_user_id/contract 파생). 멱등=draft.status+idempotency_key(FOR UPDATE). audit actor GUC(app.audit_actor_*) 운영자 귀속. schemas `operator-onboarding.ts`(예약어/슬러그/사업자번호 SSOT+7단계 payload zod snake_case strict+부서 토폴로지+멱등 입력) + lib `operator/tenant-registration/{permissions,queries,actions}`(토큰 유틸/createInvitation 재발송 재사용, Resend 미설정→activation URL 반환 KI-103). **듀얼검증 PASS_BOTH**(evaluator 8.88 / codex 1차 FAIL[P1×2 draft 무결성·sendInvite 직원초대 오염 + P2×2 modules subset·abandoned draft + P3×2] → hotfix 94ceec4 → 재검증 PASS). turbo 21/21 + 단위(operator-onboarding 22 / web permissions 3) + openapi 6 신규 + staging mig 42 적용·전 시나리오 실증(정상/멱등 replay·conflict/에러 P0101~P0111/부서 parent/가격 latch 9900/audit actor/accept admin_user_id/modules subset/abandoned) 후 정리. KI-123(P2 표시상태 파생+matrix, WI-037)/124(P3 Json 캐스트)/125(P3 pgTAP)/126(P3 checkAdminEmail auth.users) 등재. PR #54.
- [x] WI-036-feat OP-04 7단계 마법사 UI (Stepper + 실시간검증 + 임시저장 재진입 + 초기데이터) + 관리자 활성화 [ST-006/010] — codex 2라운드 협의. 라우트 `(operator)/operator/tenants/new`: 서버 page(force-dynamic, 세션/role 가드 + getOpenDraft/getRegistrationPlans 초기 fetch) → `'use client'` wizard(useReducer). queries server-only → route-local thin `'use server'` wrapper 경유. 실시간 검증 300ms debounce + per-field seq token(stale 무시) — slug/사업자번호/관리자이메일. 임시저장: form_data._wizard(RPC _submission 분리) + idempotency_key 1회 생성, 단일 save queue(병렬 금지) + 제출 전 drain + frozen hard-stop(완료 후 stale autosave 신규 draft insert 차단). 6단계 초기데이터: 부서(ui_id→parent_code 토폴로지)/근무정책/휴가종류/결재라인(default_line만, conditions=[] — DSL은 WI-034 소유)/문서양식. plan_id fetched 화이트리스트 강제(위조 차단). 와이어프레임 invite_failed/pending_invite/scheduled stale(원자 트랜잭션·enum 무변경) — 성공 화면=등록완료+activation URL 복사(평문 미저장)+sendInvite 재발급, 새로고침 시 sessionStorage recovery. ST-010 활성화는 기존 /activate 흐름. 순수 헬퍼 `lib/operator/tenant-registration/wizard.ts`(단계검증/payload빌더/draft serde/청구액) + i18n screens.op-04 ko/en 168 leaf 대칭. **듀얼검증 PASS_BOTH**(evaluator 8.15[기능8.5/품질8.5/테스트7.5/계약8.0] / codex 1차 FAIL[P2×2 autosave 루프·plan 화이트리스트 + P3 review async]+evaluator P2[startNew 멱등키]+P3[dead export] → hotfix 2bb4ee7[rev/savedRev 카운터·planAllowed·allAsyncOk·멱등키 재발급·dead export 제거] → 재검증 PASS). turbo 21/21(web 112[wizard 단위 25 신규]) + E2E 비인증 가드 2 + operator 시드 게이트. KI-127~130(P3) 등재. PR #55.
- [x] WI-037-feat OP-02 테넌트 목록 (DataTable/FilterBar + 검색/필터/sort/페이지/CSV) + 상태변경 audit [ST-007/009] — codex 3R 협의. 라우트 `(operator)/operator/tenants` 서버 page(force-dynamic, 세션/role 가드 + searchParams 파싱) → client island. `lib/operator/tenant-list/{list(순수 헬퍼: parse/sanitize/displayStatus 파생/전이검증/월요금/CSV),queries(server-only, N+1 회피 IN 병합 + deleted_at 제외),actions(상태변경 operator_super optimistic guard + trigger·service_role semantic audit/CSV export),permissions}`. DataTable/FilterBar(WI-030) 소비 + 검색(name/slug/business_number ILIKE + 사업자번호 normalize) + status·plan 필터 + 정렬 + 페이지네이션(out-of-range 보정) + UTF-8 BOM CSV(의존성0). **KI-123 표시상태 read-side 파생**(DB status≠active > pending_invite[admin_user_id NULL+대표 미만료 pending 초대] > scheduled[계약시작일>오늘 KST] > active, enum 무변경) 해소. 상태변경 audit(방식1, **DB 스키마 변경 없음**): operator 세션 UPDATE(트리거 mig29 자동) + service_role audit_logs semantic row(tenant_id=NULL → reason operator-only). i18n screens.op-02 ko/en + matrix.json Tenant C/R/U done. **듀얼검증 PASS_BOTH**(evaluator 8.38[기능8.5/품질9.0/테스트7.5/계약8.5] / codex 1차 FAIL[P1 RLS 비대칭→KI-131 + P2×4 만료초대·사업자번호검색·dead link·page보정 + P3×4]→hotfix 526f0a0→2차 P2[soft-delete]→hotfix b35b890→3차 PASS). turbo 21/21 + 단위 37(list 32/permissions 5) + E2E 비인증 가드 2. KI-131(P2 accepted-risk)/132/133/134(P3) 등재. 범위경계: change-plan/admin·Realtime 세션무효화는 WI-038. PR #56.
